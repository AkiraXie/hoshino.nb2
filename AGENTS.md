...
- 任务完成后按 §1.5 策略主动本地提交；不主动 push、不重写 Git 历史。
...
按改动的影响范围与风险分级，选择对应的工作方式。**任务开始时先判级**，并向用户
说明采用哪种工作方式；判级依据以「影响面」为主（是否波及其他模块、是否需要全仓库
验证、是否属于公共基建设计），而非改动行数。

| 级别 | 触发条件（满足其一） | 工作方式 | 提交策略 |
| --- | --- | --- | --- |
| **L0 琐碎** | typo、注释、单行格式、纯文档；无行为变化 | 当前分支直接改 | 并入相关 commit 或单独 `chore`/`style` commit |
| **L1 常规** | 单文件或单插件内修复，行为局部，验证范围明确 | 当前分支直接改 | 完成后**主动提交**（lint + 相关测试 + `git diff --check` 通过后） |
| **L2 重大** | 跨模块改动、影响多个插件、新功能、配置/依赖变更、测试基建改动 | **切分支**（`feat/xxx`、`fix/xxx` 或 `style/xxx`，从最新主分支切出） | 分支上分逻辑 commit；完成并验证后由用户确认再合并 |
| **L3 公共领域** | 公共基建（`core/`、`platform/`、`content/`、`util/`、`ai/`）的设计或代码重构、ruff 基线/全仓库格式化、依赖方向或架构契约变更 | **切分支 + `git worktree` 切出独立工作区** | worktree 中分逻辑 commit；验证通过后迁回主仓库（见下） |

流程与补充规则：

- **提交时机**：L1 及以上任务完成后主动提交，不等用户催促；一个任务拆多个逻辑
  commit（`fix`/`chore`/`test`/`style` 分开），message 用 `type(scope): 中文摘要` +
  正文说明关键点与行为变化。
- **提交门槛**（L1 及以上，提交前必须全部通过）：
  - `uv run ruff check .` 与 `uv run ruff format --check .`（或仅改动文件范围）；
  - 与改动相关的 `pytest`；公共核心/跨平台改动跑 `uv run pytest nb-tests -q` 全量；
  - `git diff --check` 无空白错误。
- **主动提交是本地提交**：不 push、不重写历史；不提交 `.env.prod`、运行日志、缓存、
  构建产物；依赖变更同时提交 `pyproject.toml` 与 `uv.lock`。
- **合并需用户确认**：L2 分支与 L3 worktree 中的提交，未经用户确认不 merge 到主分支、
  不 push。
- **worktree 约定**：
  - 位置：`/home/asp/repo/<repo>-<用途>`（如 `hoshino-review`），从主分支（如 `dev`）
    切出，保证主工作区干净；
  - 在 worktree 内完成全部改动与验证后提交；主仓库用 `git branch -f <主分支> <worktree分支>`
    或用户确认的 merge 迁移代码（worktree 与主仓库共享 `.git`，提交立即可见）；
  - 迁移后 worktree 可保留（`agent-plan-report/` 等 gitignored 报告随 worktree 保留）
    或由用户决定 `git worktree remove` 删除。
- **报告与提交解耦**：`agent-plan-report/` 中的 plan/审查/执行报告不随代码提交；
  执行报告记录提交 hash 供追溯。
- **并行协作**：L2/L3 大任务可派多个子代理并行，但每个子代理必须限定严格的文件边界
  （按目录/文件列表），防止同 worktree 并发写冲突；公共 helper（如 `hooks.spawn()`）
  先由指定代理落地，其余代理引用；跨代理交接项通过 `send_message` 传递。
- **提交前复核**：`git status --short` 与 `git diff` 检查，确认无秘密、无无关改动；
  涉及公共领域的提交，建议在 commit message 中标注验证证据（测试数量、探针结果）。
...
- `nb-tests/`：NoneBot 集成与插件行为测试，是唯一进入常规 `pytest nb-tests` 的套件。
  共享 fixture 集中在 `nb-tests/conftest.py`：`_nonebot_bootstrap`（session 级 NoneBot 初始化
  + 全插件加载）、`tmp_store`（AI store 指向临时 SQLite）、`fake_ai_server`（本地 fake
  OpenAI/Anthropic HTTP 服务器，可用 parametrize 注入自定义响应）、`_clear_uninfo_cache`；
  平台事件构造与消息 helper 见 `_helpers.py` / `adapter_events.py`。
- `.tests/`：legacy 与微博业务专项套件（`pytest .tests -q`），与 `nb-tests` 独立。
- `nb-tests/one-shot/`：真实 provider / 第三方站点联网探针（如 `test_ai_provider_live.py`、
  `live_ai_persona_probe.py`）。必须设 `ONE_SHOT_LIVE=1` 才运行，不进常规 `pytest nb-tests`；
  只打印脱敏信息，不落 key/token，不写 usage 事件。

### 8.2 测试范围与跑法（按改动风险递增）

1. 纯函数或单模块改动：`pytest <对应测试文件> -q`。
2. Service、matcher、hook、消息或平台 facade：`pytest <相关 nb-tests> -q`，至少覆盖涉及
   的 adapter。
3. 插件行为：通过真实 NoneBot dispatch 路径（事件 → `handle_event` → 断言发送/API 边界）。
4. 公共核心或跨平台改动：`pytest nb-tests -q` 全量。
5. 前端改动：`npm ... run build`，并用 Playwright 在桌面和移动 viewport 验证关键流程。
6. 涉及真实 provider/外网的验证用 one-shot 探针（§8.1），不混入常规套件。

提交前一律过 §10 门槛（`git diff --check`、ruff check/format、相关 pytest）。

### 8.3 测试纪律（增删测试的准入）

1. **计划阶段先声明补测**：任务开始时（计划/方案阶段）就要主动说明本次「是否需要补测试、
   补哪些、按哪个层级（单测/端到端/探针）」；不能在实施中途才自行决定补测。只有改动面
   超出预期（如发现隐藏的复杂分支、真实 bug 复现）才允许在实施中补充测试，且必须向用户
   说明补充理由。
2. **不为简单改动补测试**：简单 bug、简单逻辑、薄的导入/导出或纯配置读取改动，默认不新增
   测试，除非用户明确要求。这类改动靠既有测试 + lint/build + 运行探针验证即可。
3. **单测有门槛**：只有当改动涉及**复杂 bug 修复**或**流程核心业务逻辑**（解析层、权限、
   生命周期、状态机、护栏等）才写单测；单测聚焦到**函数/类/功能的整体行为**，不为单个
   if 分支、单个报错文案或单条配置项单独建用例。写完单测必须在交付说明末尾附**测试用例表**，
   逐条列出：测试文件、测试函数、测试内容、**补测理由**；理由不充分的用例不写。
4. **端到端必须是完整流程**：端到端测试跑通「从交互输入到最终产出」的一套完整流程（例如
   ``#消息 → 真实 build_agent → provider HTTP → 渲染 → 发送``），验证整套链条，而不是只测
   流程中的某一个切片；能用端到端覆盖的行为不要退化成碎片单测。

### 8.4 什么是碎片（不补、并删）

2026-08 全量排查（501 → 435 用例）确立的删除对象，新增测试时对照规避：

- 简单工具函数（几行字符串/算术/归一化/字段映射）与薄 CRUD roundtrip；
- 纯配置读取、默认值、环境变量解析断言；文案与展示断言（help/model 展示/prompt 关键词）；
- 单个 if 分支、单个报错文案、单条配置项校验的独立用例；
- import 可用性 smoke、与既有端到端重复的 rule/切片测试；
- 对第三方库逐字段透传的薄包装测试。

保留对象：

- 复杂 bug 回归（有真实故障背景）；核心状态机/权限/护栏/安全边界（SSRF、containment、
  敏感路径、并发租约）；
- 解析层与消息队列的整体行为；完整流程端到端（插件从输入到产出）；
- 平台协议关键行为：Milky 消息测试按 `docs/milky-plugin-test-protocol.md` 执行——
  `json_to_event()` 构造真实事件、唯一 `message_seq`、`handle_event()` 真实 dispatch、
  stub adapter HTTP/API 边界、断言 action/target/payload、不连接真实 QQNT/Milky endpoint。
- 规则必须有负例（§8.3 之外）：权限、scope、`only_to_me` 等规则同时覆盖「应响应」与
  「不应响应」路径；不要把「成功 import」当作行为覆盖。

### 8.5 验证与报告

- 当前测试可能暴露与任务无关的既有失败：先单独重跑并检查相关文件 diff，不要为了全绿而
  顺手修改无关业务。最终报告应给出通过数量、失败用例和为何判断其是否相关。
- 写了单测必须在交付说明末尾附**测试用例表**（§8.3.3）。
- 测试基建改动（fixture、helper、one-shot 探针、conftest）在交付说明中说明影响面与验证方式。
