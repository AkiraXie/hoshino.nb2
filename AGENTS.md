...
- **不为简单改动补测试**：简单 bug、简单逻辑、薄的导入/导出或纯配置读取改动，默认不新增
  测试，除非用户明确要求。这类改动靠既有测试 + lint/build + 运行探针验证即可。
- **单测有门槛**：只有当改动涉及**复杂 bug 修复**或**流程核心业务逻辑**（解析层、权限、
  生命周期、状态机、护栏等）才写单测；单测聚焦到**函数/类/功能的整体行为**，不为单个
  if 分支、单个报错文案或单条配置项单独建用例。写完单测必须在交付说明末尾附**测试用例表**，
  逐条列出：测试文件、测试函数、测试内容、**补测理由**；理由不充分的用例不写。
- **端到端必须是完整流程**：端到端测试跑通「从交互输入到最终产出」的一套完整流程（例如
  ``#消息 → 真实 build_agent → provider HTTP → 渲染 → 发送``），验证整套链条，而不是只测
  流程中的某一个切片；能用端到端覆盖的行为不要退化成碎片单测。one-shot 手工探针（真实
  provider 联网）按 `nb-tests/one-shot/` 现有约定管理，不进常规 `pytest nb-tests`。
