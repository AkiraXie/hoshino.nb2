...
export default function Favorites() {
  ...
  const cached = useRef(
    navType === "POP" ? loadListCache<ListData>("favorites") : null
  ).current;
  ...
  useEffect(() => {
    if (posts.length > 0) saveListCache("favorites", { posts, page, total });
  }, [posts, page, total]);
  ...
  return (
    <>
      <div className="text-lg font-semibold mb-4 pl-1 flex items-center gap-2.5">我的收藏</div>
      <div className="text-sm text-text-secondary/50 mb-6">{total} 条</div>
