...
def heartbeat(task_run_id: str, expiry: float, now: float | None = None) -> bool:
    """续租：仅当 lease 仍归当前 owner 且未过期时更新。"""
    now = now or _now()
    with ai_store.Session() as session:
        row = session.get(AITaskRun, task_run_id)
...
def outbox_mark_retry(
    outbox_id: int, error: str, next_retry_at: float | None = None
) -> None:
    with ai_store.Session() as session:
        row = session.get(AITaskOutbox, outbox_id)
        if row is not None:
