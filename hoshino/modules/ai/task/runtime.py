...
from .. import context, harness, providers, runner
...
async def run_task_run(
    ...
    *,
    ...
    """
    provider_config = config.get_provider(ctx.provider_id)
    if provider_config is None:
        raise TaskRuntimeError(f"provider {ctx.provider_id} 不存在")
    agent = providers.build_agent(ctx.provider_id, provider_config, proxy=config.proxy)
