...
async def _scheduler_loop() -> None:
    while not _stop.is_set():
        try:
            ...
            sv.logger.warning(f"task scheduler tick error={type(exc).__name__}")
...
async def _execute_claimed(run: dict, task: dict, ctx: TaskContext) -> None:
    ...
    def on_event(ev) -> None:
        ...
        if now - last_hb >= _HEARTBEAT_INTERVAL:
            task_store.heartbeat(run["id"], now + _LEASE_SECONDS, now=now)
            last_hb = now
...
async def _handle_failure(
    ...
    if current.get("attempt", 1) < _MAX_ATTEMPTS:
        ...
        sv.logger.warning(
            ...
            f"from={run['attempt']} error={type(exc).__name__}"
        )
        return

    task_store.update_run_state(run["id"], "failed", last_error=type(exc).__name__)
    task_store.update_task_status(
...
def resolve_approval(approval_id: str, state: str, resolved_by: str) -> dict:
    ...
    if appr["creator_id"] and resolved_by != appr["creator_id"]:
        return {
            "ok": False,
