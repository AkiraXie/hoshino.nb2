...
.md-body h1 {{ font-size: 1.6em; border-bottom: 1px solid var(--border); padding-bottom: 0.3em; }}
...
def build_full_html(html_body: str, theme: str = "light") -> str:
    """把渲染好的 HTML 包进带内嵌 CSS 的完整页面。"""
    theme_values = _THEMES.get(theme, _THEMES["light"])
    base_css = _BASE_CSS.format(**theme_values)
...
async def render_markdown(markdown_text: str, config: AIConfig) -> bytes:
    ...
    html = build_full_html(html_body, config.render_theme)
    browser = await get_b()
    page = await browser.new_page(viewport={"width": 820, "height": 100})
    try:
