...
def resolve_tools(deps: AgentDeps) -> list[Tool | Callable[..., Any]]:
    ...
    - chat：category 绑定（从未配置时用安全默认）+ surface + live-event + 静态风险过滤。
    """
    ...
    bindings = store.list_scope_tool_bindings(scope_key, deps.surface)
    if not bindings:
        enabled_categories = _DEFAULT_CATEGORIES
    else:
        enabled_categories = {b["category"] for b in bindings if b["enabled"]}
...
def freeze_tool_profile(scope_key: str) -> dict[str, int]:
    ...
    """
    bindings = store.list_scope_tool_bindings(scope_key, "task")
    if not bindings:
        enabled_categories = _DEFAULT_CATEGORIES
    else:
        enabled_categories = {b["category"] for b in bindings if b["enabled"]}
...
def _tool_risk(tool_name: str, kwargs: dict[str, Any]) -> RiskLevel:
    ...
    for reg in REGISTRATIONS:
        if reg.tool is None:
            continue
        name = getattr(reg.tool, "name", None) or getattr(
            reg.tool, "__name__", None
        )
