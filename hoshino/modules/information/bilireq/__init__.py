...
from .utils import  DynamicDB as db, LiveDB, get_dynamic, get_live_status, get_new_dynamic, get_user_name
    ...
    for gid in groups:
        ...
        for row in rows:
            ...
            for dyn in dyns:
                ...
                bot = groups[gid][0]
                await bot.send_group_msg(group_id=gid, message=await dyn.get_message(sv.logger))
                db.replace(group=gid, uid=uid, time=dyn.time,
...
async def _():
    ...
    if need_send:
        for uid in need_send:
            gids = [row.group for row in LiveDB.select(
                LiveDB.group).where(LiveDB.uid == int(uid))]
