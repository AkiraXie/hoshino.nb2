...
class AIConversationEvent(Base):
    ...
    为 surface 事件名（见 ``context``），``data_json`` 为其 payload（JSON）。
    只追加、不更新；``clear`` 走整段删除。
    """
...
def get_provider_row(provider_id: str) -> dict[str, Any] | None:
    with Session() as session:
        row = session.get(AIProvider, provider_id)
        return _provider_to_dict(row) if row is not None else None


def has_provider_row(provider_id: str) -> bool:
    return get_provider_row(provider_id) is not None
